        ...
        if key not in fields and values:
            ...
            if key == 'type':
                if value =='new':
                    ...
                    value = 'existing'
            if key == 'contract_series':
                value = conn.get_model("contract.series").search([('code','=',value)])
            if key == 'requirement':
                if value =='U':
                    value = 'urgent'
                else:
            ...
            if key =='required_date':
                if value == 'NULL' or value == '' or value == '00:00.0' or value == '  ':
                    ...
                    value=datetime.datetime.strptime(value, '%d/%m/%Y').strftime("%Y-%m-%d 00:00:00")
            if key == 'employee_id':
                user = conn.get_model("res.users").search([('user_code','=',value)]) 
                emp = conn.get_model("hr.employee").search([('user_id','=',user[0])])
                emp_name = conn.get_model("hr.employee").read(emp,['name'])[0]['name']
                value = emp_name
...
#        fiscalyear_pool = connection.get_model("account.fiscalyear")
        mapping = {
            "contract_series_id":"JOBSERIES",
            "employee_id": "INDENTOR",
            "name": "JOBNO",
            "maize": "JOBNO",
            "indent_date": "JOBDATE",
