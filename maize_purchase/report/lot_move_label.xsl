...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:fo="http://www.w3.org/1999/XSL/Format">
    ...
    <xsl:template match="lot-line" mode="story">
        <para style="nospace">Party Name : [<xsl:value-of select="party_code"/>] <xsl:value-of select="party_name"/></para>
        <para style="nospace">Today's Date : <xsl:value-of select="today"/></para>
        <para style="nospace">PO No.: <xsl:value-of select="purchase_name"/> Date :<xsl:value-of select="purchase_date"/></para>
        <para style="nospace">Material Description :[<xsl:value-of select="code"/>]<xsl:value-of select="product"/><xsl:text> </xsl:text><xsl:value-of select="variant"/></para>
        <para style="nospace">Qty: <xsl:value-of select="quantity"/></para>
