...
class gate_pass(report_sxw.rml_parse):
    ...
    def _user_name(self):
        ...
        res = res_user_obj.browse(self.cr,self.uid,self.uid)
        return res.name or ''
    ...
    def _user_sign(self):
        ...
        res = res_user_obj.browse(self.cr,self.uid,self.uid)
        emp_id = hr_emp.search(self.cr,1,[('name','=',res.name)])
        if emp_id:
            emp_obj = hr_emp.browse(self.cr,1,emp_id)[0]
            data_user.update({'designation':emp_obj.job_id and emp_obj.job_id.name or ''})
        data_user.update({'sign':res.sign or default_image})
        return data_user

    def _manager_sign(self):
        data_user = {}
        res_obj = self.pool.get('res.users')
        hr_emp = self.pool.get('hr.employee')
        user_id = res_obj.search(self.cr,self.uid,[('login','=','asd')])
        res_record = res_obj.browse(self.cr,self.uid,user_id)[0]
        emp_id = hr_emp.search(self.cr,1,[('name','=',res_record.name)])
        if emp_id:
            emp_obj = hr_emp.browse(self.cr,1,emp_id)[0]
            data_user.update({'designation':emp_obj.job_id and emp_obj.job_id.name or ''})
