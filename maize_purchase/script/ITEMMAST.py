...
def get_field_mapping(conn, values, mapping):
    ...
    for key,val in mapping.items():
        if key not in fields and values:
            ...
            if key =='last_po_date':
                import datetime
                if value == 'NULL' or value == '' or value == '00:00.0' or value == '  ' or value == ' ':
                    value = ''
                else:
            ...
            if key == 'last_supplier_code/.id':
                ...
                if supplier == [] or supplier == False:
                    un_sup = conn.get_model("res.partner").search([('supp_code','=','1111111'),('name','=','Undefine Supplier')])
                    if not un_sup:
                        un_sup = [conn.get_model("res.partner").create({'name':'Undefine Supplier','supp_code':'1111111','supplier':True,'active':True})]
                    value=un_sup[0]
                else:
                    value=supplier[0]
            data_lst.append(value)
        #add defalut service product
        if key == 'default_code':
            fields.append('type')
            if value[0:4]=='0192':
                data_lst.append('service')
            else:
                data_lst.append('product')
        ...
        if key == 'default_code':
            sub = conn.get_model("product.sub.group").search([('major_group_id','=',value[2:4]),('code','=',value[4:6])])
...
def main():
    ...
    try:
        ...
        data_lines = read_csv_data(root_path + "ITEMMAST_2nd march.csv")
