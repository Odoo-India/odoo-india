    ...
    for key,val in mapping.items():
        if key not in fields and values:
            ...
            if key == 'product_id':
                value='0'+value
            if key == 'indent_id':
                aa = conn.get_model("indent.indent").search([('maize','=',value)])[0]
                value = conn.get_model("indent.indent").read(aa,['name'])['name']
            if key == 'product_uom':
                prd = conn.get_model("product.product").search([('default_code','=','0'+value)])[0]
                uom = conn.get_model("product.product").read(prd,['uom_id'])['uom_id'][1]
                value = uom
            data_lst.append(value)
    print "fields, data_lst", fields, data_lst
...
#        fiscalyear_pool = connection.get_model("account.fiscalyear")
        ...
        print "Contract transaction import successfully",p, r, warning, s
...
def main():
    ...
    try:
        ...
        data_lines = read_csv_data(root_path + "CONTRACT.csv")
