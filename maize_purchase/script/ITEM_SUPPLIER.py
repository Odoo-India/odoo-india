        ...
        if key not in fields and values:
            ...
            if key == 'name/.id':
                ...
                if supplier == [] or supplier == False:
                    ss = conn.get_model("res.partner").search([('supp_code','=','1111111')])
                    value=ss[0]
                else:
                    value=supplier[0]
            if key == 'product_id/.id':
                value = '0'+value
                product = conn.get_model("product.product").search([('default_code','=',value)])
                if product == [] or product == False:
                    value=''
                else:
                    value=product[0]
...
class Parser(object):
    ...
    def import_postcode(self, lines):
        ...
        postal_pool = connection.get_model("product.supplierinfo")
#        fiscalyear_pool = connection.get_model("account.fiscalyear")
        mapping = {
            "name/.id": "LSUPPCODE",
            "product_id/.id": "ITEMCODE",
        }
        values = []
        for line in lines:
        ...
        print "Product supplier info import successfully",p, r, warning, s
